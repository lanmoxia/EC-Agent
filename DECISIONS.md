...
**我的自评（不护短）**：「完成即接力推送」部分有效但同源风险：
- 仅**锚点行**被 Stop 钩子硬校验；**钩子不检查我是否真的 commit/relay 了** → 可像前缀一样静默腐化。
- 粒度模糊（"小任务"无定义）→ 漂移。
- **只在完成后 checkpoint**：任务中途被打断（如本次 503 崩在 PROJECT-OVERVIEW 写完、清单步没插）→ 完成日志里看不到半成品进度。
- 每微步 commit 污染历史。

**用户提案**：任务进度文件（backlog）+ 当前 todo 指针，两文件；中断后读两文件即知做到哪。
**评估**：正好补最痛的洞（把"当前位置"外置→中途中断可恢复），优于"只记完成"。但**状态文件自身有腐化风险**：不刷新就误导，故同样需强制更新而非靠我自觉。

**核心原则（从衰减的前缀学到）**：稳健 = ①状态在**文件**里(不在会话上下文) + ②更新由**代码(钩子)**驱动(不靠我纪律)。凡靠"我记得"的都会衰减。

**我的推荐（待用户定）＝分层，让钩子写、不靠我**：
- **L1 每轮(钩子强制,零纪律)**：Stop 钩子已每轮解析锚点 current_task/next → **扩展它把锚点自动写进 `PROGRESS.md`**。"当前位置"成为已强制锚点的免费副产物（锚点＝被钩子托住的"前缀"复活版）。抓中途中断。
- **L2 任务边界(耐久checkpoint)**：git commit/push 防换机/丢盘，在自然任务边界、非每微步。
...
## ❌ 视频去硬字幕功能（已取消，2026-06-24）

> **取消决策（2026-06-24，用户拍板）**：现阶段模型处理不可用，去硬字幕任务整体消除，从 TASKS 待办移除。下方调研/de-risk 细节保留沉淀，日后模型/工具成熟再议。
...
**现状**：❌ 已取消（2026-06-24，现阶段模型处理不可用；de-risk 成果与恢复步骤保留，留待日后）。
